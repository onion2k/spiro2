...
export function renderInstancedSprites(options: RenderInstancedSpritesOptions) {
  const {
    runtimeLayer,
    center,
    nowSec,
  ...
  for (let i = Math.max(step, 1); i < runtimeLayer.trail.length; i += step) {
    ...
    const spriteSize = Math.max(
      0.6,
      lineWidthForPoint(current, strokeWidthMode, baseLineWidth, lineWidthBoost) * 1.35
    )
