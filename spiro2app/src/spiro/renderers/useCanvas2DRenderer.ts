...
import type { LayerConfig, Point, TrailPoint } from '../types'
import type { SpiroRendererConfig } from './types'

type Canvas2DRendererOptions = SpiroRendererConfig & {
  canvasRef: RefObject<HTMLCanvasElement | null>
  enabled: boolean
}

export function useCanvas2DRenderer(options: Canvas2DRendererOptions) {
  const {
    canvasRef,
    enabled,
    layers,
  ...
  useEffect(() => {
    ...
    const runtimeLayers = layers.map((layer) => ({
      ...
      paramT: 0,
      paramU: 0,
      previous: null as Point | null,
      previousAngle: null as number | null,
      trail: [] as TrailPoint[],
      pointIndex: 0,
      sampleCounter: 0,
      bakedPointCount: 0,
    ...
    const buildSymmetryVariants = (point: Point, center: Point) => {
      ...
      for (let i = 0; i < repeats; i += 1) {
        ...
        const baseVariants: Point[] = [rotated]
    ...
    const buildLineOffsets = (layer: LayerConfig, pointIndex: number, paramU: number) => {
      ...
      const normalizedPhaseStep =
        (0.08 * layer.multiLineMotionSpeed) / Math.max(0.15, Math.abs(layer.speed))
    ...
    const colorForPoint = (point: TrailPoint, layer: LayerConfig, nowSec: number) => {
    ...
    const drawLayerRange = (
      targetContext: CanvasRenderingContext2D,
      runtime: (typeof runtimeLayers)[number],
      nowSec: number,
      center: Point,
      startIndex: number,
      endIndex: number
              ...
              if (runtime.previous && dt > 0) {
                ...
                if (runtime.previousAngle !== null) {
                  const turn = Math.atan2(
                    Math.sin(angle - runtime.previousAngle),
                    Math.cos(angle - runtime.previousAngle)
                  )
