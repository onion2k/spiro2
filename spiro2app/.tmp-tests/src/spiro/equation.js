export function compileParametric(exprX, exprY, exprZ) {
    try {
        const compiled = new Function('t', 'u', 'R', 'r', 'd', '"use strict"; const {sin,cos,tan,abs,sqrt,pow,PI,E,log,exp,min,max,atan2,sign,floor} = Math; const clamp=(v,lo,hi)=>min(max(v,lo),hi); const mix=(a,b,p)=>a+(b-a)*p; const saw=(v)=>2*(v/(2*PI)-floor(v/(2*PI)+0.5)); const triangle=(v)=>2*abs(saw(v))-1; const pulse=(v,w)=>abs(saw(v))<w?1:0; return { x: ' + exprX + ', y: ' + exprY + ', z: ' + exprZ + ' };');
...
export function compileParametricGenerator(exprX, exprY, exprZ) {
...
export function compileLissajousGenerator(layer) {
    const generator = {
        kind: 'lissajous',
        createState: () => ({ t: 0, u: 0 }),
...
export function compileStrangeAttractorGenerator(layer) {
    const generator = {
...
export function compileLayerGenerator(layer) {
